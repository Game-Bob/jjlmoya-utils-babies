...
src/tool/mi-tool/
  ...
  seo.astro          ← lee content.seo del i18n y renderiza con switch
...
## 4. CSS — clases modificadoras sin doble guion

El linter (stylelint-config-standard) no permite `--` en nombres de clase.
Usa guion simple para todos los modificadores de estado:

```css
/* ERROR — doble guion no permitido */
.mi-tool-btn--active { }
.mi-tool-card--selected { }
.mi-tool--t1 { }

/* CORRECTO — guion simple */
.mi-tool-btn-active { }
.mi-tool-card-selected { }
.mi-tool-t1 { }
```

Lo mismo en el JS del componente:

```ts
el.classList.add('mi-tool-btn-active');      // correcto
el.classList.toggle('mi-tool-card-selected', isSelected);  // correcto
```

---

## 5. Stylelint — errores exactos que vas a ver
...
## 10. seo.astro — patron OBLIGATORIO

**`seo.astro` NO tiene contenido hardcodeado.** Lee el array `seo` del i18n y lo
renderiza con un switch. El contenido SEO va en los ficheros `i18n/es.ts`, `en.ts` y `fr.ts`.
...
import { miTool } from './index';
import type { KnownLocale } from '../../types';

interface Props { locale?: KnownLocale; }
const { locale = 'es' } = Astro.props;
const content = await miTool.i18n[locale]?.();
...
El array `seo` en los i18n usa objetos con `type` como discriminante:

```ts
seo: [
  { type: 'summary', title: 'Resumen', items: ['punto 1', 'punto 2'] },
  { type: 'title', text: 'Titulo H2', level: 2 },
  { type: 'paragraph', html: 'Texto con <strong>HTML</strong> permitido.' },
  { type: 'stats', columns: 3, items: [
    { value: '150 ml/kg', label: 'Referencia diaria' },
  ]},
  { type: 'tip', html: 'Consejo importante.' },
  { type: 'table', headers: ['Col1', 'Col2'], rows: [['val1', 'val2']] },
  { type: 'list', items: ['item 1', 'item 2'] },
  { type: 'comparative', columns: 2, items: [
    { title: 'Opcion A', description: 'Descripcion A', points: ['punto 1'] },
    { title: 'Opcion B', description: 'Descripcion B', points: ['punto 1'] },
  ]},
  { type: 'glossary', items: [
    { term: 'Termino', definition: 'Definicion del termino.' },
  ]},
],
```

> **NUNCA dejes `seo: []` vacio.** Si el array esta vacio, la seccion SEO de la pagina
> no muestra nada. El SEO debe estar en los tres ficheros de idioma.

---

## 11. Categoria y data.ts
...
## 12. Checklist — ejecutar antes de `npm run minor`

Puedes pasarle este checklist a Claude en la proxima sesion junto con el codigo.

### SEO
- [ ] `seo.astro` sigue el patron switch que lee del i18n — sin contenido hardcodeado
- [ ] El array `seo` en `es.ts`, `en.ts` y `fr.ts` tiene contenido real (nunca `seo: []`)
- [ ] Las clases modificadoras usan guion simple (`-active`, `-boy`) — **nunca doble guion** (`--active`, `--boy`)
